...
/* variables used by various homography estimation routines */
struct Hdata{
  double **cfs;
...
static int est2DPtHomog(double *h, int npts, int *ptsidx, void *adata)
{
register int i, j;
double *mat, *coefs;
...
struct Hdata *dat=(struct Hdata *)adata;
  ...
  if(h==NULL){ /* cleanup */
    ...
    /* free working memory */
    free(dat->M);
    dat->M=dat->b=NULL; dat->Mrows=0;
    return 0;
  }

  if(dat->Mrows!=2*npts){ /* free existing matrix and allocate a new one */
...
static void homogResidualsAlg(double *homog, int numres, void *adata, double *resid)
{
register int i, j;
...
static void homogResidualsGeom(double *homog, int numres, void *adata, double *resid)
{
register int i;
...
double (*pts0)[2]=dat->pts0;
double (*pts1)[2]=dat->pts1;
...
/* nonlinear refinement of a homography as specified by "howto":
 *  HOMEST_SYM_XFER_ERROR: minimize symmetric transfer error
 *  HOMEST_XFER_ERROR2: minimize transfer error in 2nd image
 *  HOMEST_XFER_ERROR1: minimize transfer error in 1st image
 *  HOMEST_SAMPSON_ERROR: minimize the Sampson distance
 *  HOMEST_REPR_ERROR: minimize the reprojection error
 */
static void refine2DPtHomog(double *h, int howto, struct Hdata *data, int verbose)
{
register int i, j;
...
static char *howtoname[]={"HOMEST_NO_NLN_REFINE", "HOMEST_XFER_ERROR/HOMEST_XFER_ERROR2",
  ...
  switch(howto){
    case HOMEST_XFER_ERROR:
      m=NUM_HPARAMS; n=2*data->numInliers; // two measurements per point pair
      if((x=(double *)malloc(n*sizeof(double)))==NULL){
        fprintf(stderr, "Memory allocation request failed in refine2DPtHomog()\n");
        exit(1);
      }
      for(i=0; i<data->numInliers; ++i){
        j=i<<1; // 2*i
        x[j]  =data->pts1[data->inliersidx[i]][0];
        x[j+1]=data->pts1[data->inliersidx[i]][1];
      }
      p=h;
      err=HomXfer;
      jacerr=jacHomXfer;
    ...
    case HOMEST_SYM_XFER_ERROR:
      m=NUM_HPARAMS; n=4*data->numInliers; // four measurements per point pair
      if((x=(double *)malloc(n*sizeof(double)))==NULL){
        fprintf(stderr, "Memory allocation request failed in refine2DPtHomog()\n");
        exit(1);
      }
      for(i=0; i<data->numInliers; ++i){
        j=i<<2; // 4*i
        x[j]  =data->pts0[data->inliersidx[i]][0];
        x[j+1]=data->pts0[data->inliersidx[i]][1];
        x[j+2]=data->pts1[data->inliersidx[i]][0];
        x[j+3]=data->pts1[data->inliersidx[i]][1];
      }
      p=h;
      err=HomSymXfer;
    ...
    case HOMEST_SAMPSON_ERROR:
      m=NUM_HPARAMS; n=data->numInliers; // one measurement per point pair
      if((x=(double *)malloc(n*sizeof(double)))==NULL){
        fprintf(stderr, "Memory allocation request failed in refine2DPtHomog()\n");
        exit(1);
      }
    ...
    case HOMEST_REPR_ERROR:
      m=NUM_HPARAMS+2*data->numInliers; n=4*data->numInliers; // four measurements per point pair
      ...
      for(i=0; i<data->numInliers; ++i){
        j=i<<1; // i*2
        p[NUM_HPARAMS+j]  =data->pts0[data->inliersidx[i]][0];
        p[NUM_HPARAMS+j+1]=data->pts0[data->inliersidx[i]][1];
      }
  ...
  if(verbose){
    ...
    fprintf(stdout, "LM returned %d in %g iter, reason %g, error %g [initial %g], %d/%d func/fjac evals\n",
                    ret, info[5], info[6], info[1]/data->numInliers, info[0]/data->numInliers, (int)info[7], (int)info[8]);
#if 0
    fprintf(stdout, "\nSolution: ");
    for(i=0; i<m; ++i)
  ...
  if(howto==HOMEST_REPR_ERROR){
    for(i=0; i<data->numInliers; ++i){
      ...
      x1/=s;
      y1/=s;
      //printf("%d: ", data->inliersidx[i]); // this displays the point's input index
...
struct Hdata dat;
  ...
  if(!dat.cfs){
    fprintf(stderr, "Error: not enough memory for 'dat.cfs' in homest()\n");
    exit(1);
  }

  /* one "big" malloc instead of several "small" ones */
  dat.cfs[0]=(double *)malloc(2*nmatches*NUM_HPARAMS*sizeof(double)); /* two equations per point */
  if(!dat.cfs[0]){
    fprintf(stderr, "Error: not enough memory for 'dat.cfs[0]' in homest()\n");
    exit(1);
  }
	for(i=1; i<nmatches; ++i)
    dat.cfs[i]=dat.cfs[i-1] + 2*NUM_HPARAMS;
  dat.rhs=NULL;

	for(i=0; i<nmatches; ++i)
    calc2DHomogLinCoeffs2(pts0[i], pts1[i], dat.cfs[i]);
  ...
  j=lqsfit(nmatches, MIN_HMATCHED_PTS, sets, nbSets, HOWTO_CALC_RESIDUALS, est2DPtHomog,
            isSqr, verbose, maxNbSol, gate, premResid, NUM_HPARAMS, inlPcent, (void *)&dat,
            H01, NULL, outliersMap, nbOutliers, &outlierThresh);
#else
  ...
  j=ransacfit(nmatches, MIN_HMATCHED_PTS, sets, nbSets, homogResidualsGeom, est2DPtHomog,
            isSqr, verbose, maxNbSol, outlierThresh, 0, NUM_HPARAMS, inlPcent, (void *)&dat, H01, NULL, outliersMap, nbOutliers);
#endif
  ...
  if(j!=0){
    ...
    if(!dat.inliersidx){
      fprintf(stderr, "Error: not enough memory for 'dat.inliersidx' in homest()\n");
      exit(1);
    }
    ...
    est2DPtHomog(H01, dat.numInliers, dat.inliersidx, (void *)&dat);
  ...
  est2DPtHomog(NULL, 0, NULL, (void *)&dat); /* release memory */
...
/* Compute the values dividing the symmetric average distance errors pertaining to a homography H
 * and a set of point pairs into four equal parts. The three values respectively cut off the lowest
 * 25%, 50% and 75% of data.
 *
 */

HOMEST_API_MOD void HOMEST_CALL_CONV
homest_quartiles(double (*inpts0)[2], double (*inpts1)[2], int nmatches, double H[NUM_HPARAMS],
...
double H1[NUM_HPARAMS], *errors, sum, q1[2], q2[2], s, dist1, dist2;
  ...
  if((errors=(double *)malloc(nmatches*sizeof(double)))==NULL){
     fprintf(stderr, "Memory allocation request failed in homest_quartiles()\n");
     exit(1);
  }
  homest_matinvLU(H, H1, 3);
  homest_matinvLU(NULL, NULL, 0);
...
static int est2DPtAffHomog(double *ah, int npts, int *ptsidx, void *adata)
{
register int i, j;
double *mat, *coefs, *b;
...
struct Hdata *dat=(struct Hdata *)adata;
  ...
  if(dat->Mrows!=2*npts){ /* free existing matrix and allocate a new one */
    ...
    dat->M=mat;
    dat->b=mat+2*npts*NUM_HAFFPARAMS;
    dat->Mrows=2*npts;
  }
...
static void affhomogResidualsAlg(double *ahomog, int numres, void *adata, double *resid)
{
register int i, j;
...
struct Hdata dat;
  ...
  dat.cfs=(double **)malloc(nmatches*sizeof(double *));
  if(!dat.cfs){
    fprintf(stderr, "Error: not enough memory for 'dat.cfs' in homestaff()\n");
    exit(1);
  }

  /* one "big" malloc instead of several "small" ones */
  dat.cfs[0]=(double *)malloc(2*nmatches*(NUM_HAFFPARAMS + 1)*sizeof(double)); /* two equations per point + RHS */
  if(!dat.cfs[0]){
    fprintf(stderr, "Error: not enough memory for 'dat.cfs[0]' in homestaff()\n");
    exit(1);
  }
	for(i=1; i<nmatches; ++i)
    dat.cfs[i]=dat.cfs[i-1] + 2*NUM_HAFFPARAMS;
  dat.rhs=dat.cfs[0]+2*nmatches*NUM_HAFFPARAMS;

	for(i=0; i<nmatches; ++i)
    calc2DAffHomogLinCoeffs2(pts0[i], pts1[i], dat.cfs[i], dat.rhs+2*i);
  ...
  j=lqsfit(nmatches, MIN_HAFFMATCHED_PTS, sets, nbSets, affhomogResidualsAlg, est2DPtAffHomog,
            isSqr, verbose, maxNbSol, gate, premResid, NUM_HAFFPARAMS, inlPcent, (void *)&dat, 
            H01, NULL, outliersMap, nbOutliers, &outlierThresh);
  ...
  if(j!=0){
    dat.inliersidx=(int *)malloc((nmatches - *nbOutliers)*sizeof(int));
    if(!dat.inliersidx){
      fprintf(stderr, "Error: not enough memory for 'dat.inliersidx' in homestaff()\n");
      exit(1);
    }
    ...
    /* LS estimation on inliers */
    dat.numInliers=nmatches - *nbOutliers;
    est2DPtAffHomog(H01, dat.numInliers, dat.inliersidx, (void *)&dat);
  ...
  est2DPtAffHomog(NULL, 0, NULL, (void *)&dat); /* release memory */
